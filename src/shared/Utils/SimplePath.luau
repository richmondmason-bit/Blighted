...
local DEFAULT_SETTINGS: PathSettings = {
...
function Path.GetNearestCharacter(fromPosition)
	local character, dist = nil, 50000
...
--[[ CONSTRUCTOR ]]--
function Path.new(agent: Model, agentParameters: {[string]: any}, override: PathSettings?)
	...
	local self = setmetatable({
		...
		_agent = agent,
		_humanoid = agent:FindFirstChildWhichIsA("Humanoid"),
    ...
    -- will add settings that haven't been specified if it changes anything from default
    if self._settings ~= DEFAULT_SETTINGS then
        for n, v in DEFAULT_SETTINGS do
            if self._settings[n] == nil then
                self._settings[n] = v
            end
        end
    end
