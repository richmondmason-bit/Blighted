...
local SmoothShiftLockSettings = require(script.Parent:WaitForChild("SmoothShiftLock"):WaitForChild("Settings"))
local Spring = require(script.Parent:WaitForChild("Spring"))
local CameraOffsetSpring = Spring.new(Vector3.new(0, 0, 0))
CameraOffsetSpring.Damper = SmoothShiftLockSettings.OFFSET_TRANSITION_DAMPER
...
function BaseCamera:SetMouseLockOffset(Enable: boolean)
	if (Enable) then
		CameraOffsetSpring.Speed = SmoothShiftLockSettings.OFFSET_TRANSITION_IN_SPEED
		CameraOffsetSpring.Target = SmoothShiftLockSettings.LOCKED_CAMERA_OFFSET
	else
		CameraOffsetSpring.Speed = SmoothShiftLockSettings.OFFSET_TRANSITION_OUT_SPEED
		CameraOffsetSpring.Target = Vector3.new(0, 0, 0)
	end
end

function BaseCamera:GetMouseLockOffset()
	return CameraOffsetSpring.Position
end
