...
local function initializeSoundSystem(instances: { [string]: Instance })
	...
	if FFlagUserNewCharacterSoundsApi and SoundService.CharacterSoundsUseNewApi == Enum.RolloutState.Enabled then
		...
		-- initialize sounds
		for name: string, props: {[string]: any} in AUDIOPLAYER_DATA do
			...
			sound.Volume = 0.65
			for propName, propValue: any in props do
		...
		-- initialize sounds
		for name: string, props: {[string]: any} in SOUND_DATA do
	...
	local loopedSoundUpdaters: {[Playable]: (number, Playable, Vector3) -> ()} = {
		[sounds.Climbing] = function(_dt: number, sound: Playable, vel: Vector3)
		...
		[sounds.Running] = function(_dt: number, sound: Playable, vel: Vector3)
	...
	local steppedConn = RunService.PreSimulation:Connect(function(worldDt: number)
	...
	local function terminate()
		...
		for _name: string, sound: Playable in sounds do
			sound:Destroy()
		end
		table.clear(sounds)
	end
...
Utils.Player.ObservePlayers(playerAdded)
