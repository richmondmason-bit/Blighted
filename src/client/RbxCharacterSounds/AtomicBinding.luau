...
function AtomicBinding.new(manifest, boundFn)
	...
	for alias, rawPath in manifest do
...
function AtomicBinding:bindRoot(root)
	...
	assert(rootInstToManifest[root] == nil, "Nil Root Instance To Manifest")
	...
	for alias, parsedPath in parsedManifest do
	...
	local function processNode(node)
		local instance = assert(node.instance, "Nil Node Instance")
		...
		if not isLeaf then
			...
			local function processDeleteChild(childInstance)
				...
				assert(childNode.instance == nil, "Nil Child Instance") -- If this triggers, unbindNodeDescend failed
...
function AtomicBinding:unbindRoot(root)
	...
	if rootNode then
		local resolvedManifest = assert(rootInstToManifest[root], "Unresolved Manifest")
...
function AtomicBinding:destroy()
	...
	for _, dtor in self._dtorMap do
		dtor:destroy()
	end
	...
	local rootInstToManifest = self._rootInstToManifest
	for rootInst, rootNode in self._rootInstToRootNode do
		local resolvedManifest = assert(rootInstToManifest[rootInst], "Unresolved Manifest")
		unbindNodeDescend(rootNode, resolvedManifest)
	end
